...
import type { DoctorSearchResult } from "./actions";

type Props = {
    initialData: DoctorSearchResult;
    specialties: string[];
...
export default function DoctorsClient({
    ...
    const updateURL = (newSearchTerm: string, newSpecialty: string, newPage: number) => {
        const params = new URLSearchParams();
        if (newSearchTerm) params.set("q", newSearchTerm);
        if (newSpecialty) params.set("area", newSpecialty);
        if (newPage > 1) params.set("p", newPage.toString());

        const query = params.toString();
        startTransition(() => {
            router.push(`/medicos${query ? `?${query}` : ""}`);
        });
    };
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSearchTerm(value);
        updateURL(value, selectedSpecialty, 1);
    };
    ...
    const handlePageChange = (page: number) => {
        updateURL(searchTerm, selectedSpecialty, page);
    ...
    const { doctors, totalCount, totalPages, currentPage } = initialData;
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <header className="mb-10 text-center md:text-left">
                    ...
                    <p className="mt-4 text-lg text-gray-600 max-w-2xl">
                        Contamos con {totalCount} especialistas altamente calificados y comprometidos con tu bienestar integral.
                    </p>
                </header>
                ...
                <div className="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-xl font-bold text-gray-900 flex items-center gap-3">
                        {isPending && <div className="animate-spin h-5 w-5 border-2 border-um-green border-t-transparent rounded-full"></div>}
                ...
                {/* Doctors Grid/List */}
                <div className="grid grid-cols-1 gap-8" style={{ opacity: isPending ? 0.6 : 1, transition: 'opacity 0.2s' }}>
                    {doctors.length > 0 ? (
                        doctors.map((doc) => (
                            <article
                                key={doc.id}
                            ...
                            >
                                ...
                                <div className="relative group/img flex-shrink-0">
                                    ...
                                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1 rounded-full bg-white px-2 py-1 shadow-md ring-1 ring-gray-100">
                                        ...
                                        <span className="text-xs font-bold text-gray-900">{(doc.rating ?? 0).toFixed(1)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-1 flex-col text-center lg:text-left">
                                    <div className="mb-2">
                                        <h3 className="text-xl font-extrabold text-gray-900 transition-colors group-hover:text-um-green">
                                            <Link href={`/medicos/${doc.externalId}`}>
                                                {doc.name}{doc.title ? `, ` : ""}<span className="text-gray-400 font-bold uppercase text-[0.7rem] ml-1">{doc.title || ""}</span>
                                            </Link>
                                        </h3>
                                    ...
                                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        ...
                                        <div className="flex items-center justify-center lg:justify-start gap-2.5 text-gray-600">
                                            ...
                                            <span className="text-sm font-semibold text-gray-700 truncate">{doc.location || "Santiago, R.D."}</span>
                                        </div>
                                    </div>
